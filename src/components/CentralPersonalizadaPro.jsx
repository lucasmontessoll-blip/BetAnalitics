...
export default function CentralPersonalizadaPro({ userData, jogos = [], setViewMode, setJogoSelecionado }) {
  ...
  const history = useMemo(() => { try { const v = JSON.parse(localStorage.getItem('Golnexa_historico_ia_v1') || '[]'); return Array.isArray(v) ? v : []; } catch { return []; } }, []);
  ...
  async function share() {
    const text = `Golnexa PRO — ${personalized.length} jogos no meu painel personalizado. Análises estatísticas, sem promessa de lucro.`;
    if (navigator.share) await navigator.share({ title: 'Golnexa PRO', text, url: location.origin }); else { await navigator.clipboard.writeText(`${text} ${location.origin}`); setStatus('Card copiado para compartilhar.'); }
  }
