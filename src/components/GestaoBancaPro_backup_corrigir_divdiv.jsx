...
function carregarEntradas() {
  try {
    const salvo = localStorage.getItem("bet_banca_entradas");
...
function CardBanca({ icon: Icon, titulo, valor, subtitulo, cor }) {
  const corFinal = cor || 'text-white';

  return (
    <div className="bg-[#0f172a] border border-white/10 rounded-3xl p-4 shadow-lg">
      <div className="flex items-start justify-between gap-3">
        <divdiv className="flex items-start justify-between gap-3">
        <div>
          <div className="text-[9px] font-black uppercase tracking-[0.16em] text-slate-500 mb-2">
            {titulo}
          </div>

          <div className={"text-xl font-black " + corFinal}>
            {valor}
          </div>
        ...
        <div className="w-10 h-10 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center">
          <Icon className={"w-5 h-5 " + corFinal} />
        </div>
      </div>
    </div>
  );
    ...
    <div className="bg-black/20 border border-white/10 rounded-2xl p-4">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div>
          ...
          <div className="text-[10px] font-bold text-slate-500 mt-1">
            {entrada.mercado} · odd {Number(entrada.odd || 0).toFixed(2)}
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 text-center">
        ...
        <div className="bg-white/5 rounded-xl p-2">
          ...
          <div className={entrada.lucro >= 0 ? 'text-xs font-black text-green-400' : 'text-xs font-black text-red-300'}>
...
export default function GestaoBancaPro() {
  const [saldoInicial, setSaldoInicial] = useState(function () {
  ...
  const [entradas, setEntradas] = useState(function () {
    return carregarEntradas();
  });

  const resumo = useMemo(function () {
    const inicial = numero(saldoInicial, 1000);
    const lucroTotal = entradas.reduce(function (acc, item) {
      return acc + numero(item.lucro);
    }, 0);

    const investido = entradas.reduce(function (acc, item) {
      return acc + numero(item.valor);
    }, 0);

    const greens = entradas.filter(function (e) {
      return e.status === 'green';
    }).length;

    const reds = entradas.filter(function (e) {
      return e.status === 'red';
    }).length;
  ...
  function registrarEntrada() {
    ...
    const novaEntrada = {
      id: Date.now(),
      jogo: jogo || 'Jogo sem nome',
      mercado: mercado || 'Mercado não informado',
      valor: valor,
      odd: oddNum,
      status: status,
      retorno: retorno,
    ...
    const novas = [novaEntrada].concat(entradas).slice(0, 20);
    ...
    <div className="px-4 animate-fade-in pb-28 w-full">
      <div className="bg-gradient-to-br from-green-500/20 via-[#0f172a] to-blue-500/10 border border-green-500/30 rounded-[32px] p-5 mb-5 shadow-2xl">
        ...
        <div className="mt-4 bg-black/20 border border-white/10 rounded-2xl p-3">
          ...
          <div className={resumo.saldoAtual >= resumo.inicial ? 'text-3xl font-black text-green-400' : 'text-3xl font-black text-red-300'}>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <CardBanca icon={Wallet} titulo="Banca inicial" valor={dinheiro(resumo.inicial)} subtitulo="valor base" />
        <CardBanca icon={resumo.lucroTotal >= 0 ? TrendingUp : TrendingDown} titulo="Lucro/Prejuízo" valor={dinheiro(resumo.lucroTotal)} subtitulo="resultado atual" cor={resumo.lucroTotal >= 0 ? 'text-green-400' : 'text-red-300'} />
        <CardBanca icon={BarChart3} titulo="ROI" valor={resumo.roi.toFixed(1) + '%'} subtitulo="retorno sobre entradas" cor={resumo.roi >= 0 ? 'text-blue-400' : 'text-red-300'} />
        <CardBanca icon={Target} titulo="Acerto" valor={resumo.taxaAcerto.toFixed(1) + '%'} subtitulo={resumo.greens + ' green / ' + resumo.reds + ' red'} cor="text-yellow-300" />
      </div>
      ...
      <div className="bg-[#0f172a] border border-white/10 rounded-3xl p-5 mb-5">
        ...
        <div className="space-y-3">
          <input value={saldoInicial} onChange={(e) => setSaldoInicial(e.target.value)} placeholder="Banca inicial" className="w-full bg-black/30 border border-white/10 rounded-2xl px-4 py-3 text-sm font-bold text-white outline-none" />
          <input value={jogo} onChange={(e) => setJogo(e.target.value)} placeholder="Jogo" className="w-full bg-black/30 border border-white/10 rounded-2xl px-4 py-3 text-sm font-bold text-white outline-none" />
          <input value={mercado} onChange={(e) => setMercado(e.target.value)} placeholder="Mercado" className="w-full bg-black/30 border border-white/10 rounded-2xl px-4 py-3 text-sm font-bold text-white outline-none" />

          <div className="grid grid-cols-2 gap-3">
            <input value={valorEntrada} onChange={(e) => setValorEntrada(e.target.value)} placeholder="Valor" className="w-full bg-black/30 border border-white/10 rounded-2xl px-4 py-3 text-sm font-bold text-white outline-none" />
            <input value={odd} onChange={(e) => setOdd(e.target.value)} placeholder="Odd" className="w-full bg-black/30 border border-white/10 rounded-2xl px-4 py-3 text-sm font-bold text-white outline-none" />
          </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {['pendente', 'green', 'red'].map((s) => (
              <button
                ...
                type="button"
                onClick={() => setStatus(s)}
                className={status === s ? 'rounded-2xl px-3 py-3 text-[10px] font-black uppercase border bg-blue-600 border-blue-400 text-white' : 'rounded-2xl px-3 py-3 text-[10px] font-black uppercase border bg-black/20 border-white/10 text-slate-400'}
              >
                {s}
              </button>
            ))}
      ...
      <div className="bg-[#0f172a] border border-white/10 rounded-3xl p-5 mb-5">
        ...
        <div className="flex items-end gap-2 h-28">
          {[].concat(entradas).slice(0, 8).reverse().map((entrada, index) => {
            ...
            return (
              <div key={entrada.id || index} className="flex-1 flex flex-col items-center gap-2">
                <div
                  className={entrada.lucro >= 0 ? 'w-full rounded-t-xl bg-green-500/70' : 'w-full rounded-t-xl bg-red-500/70'}
                  style={{ height: altura + '%' }}
                />
