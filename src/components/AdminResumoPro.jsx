...
import { carregarRetencaoAdmin } from '../services/intelligenceR59.js';
...
export default function AdminResumoPro({ setViewMode, userData }) {
  ...
  const carregar = useCallback(async () => {
    ...
    try { const p = periodo(dias); const [dashboard, quality, retention] = await Promise.all([carregarDashboard(p.from, p.to), carregarRelatosAdmin(), carregarRetencaoAdmin()]); setDados(dashboard); setRelatos(quality); setRetencao(retention.retention || {}); }
  ...
  return <main className="w-full px-4 pb-28 text-white animate-fade-in">
    ...
    <section className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5"><Card titulo="DAU" valor={numero(retencao.dau)} detalhe="ativos hoje" icon={Users}/><Card titulo="WAU" valor={numero(retencao.wau)} detalhe="ativos em 7 dias" icon={Users}/><Card titulo="MAU" valor={numero(retencao.mau)} detalhe="ativos em 30 dias" icon={Users}/><Card titulo="Frequência" valor={`${retencao.stickiness || 0}%`} detalhe="DAU ÷ MAU" icon={BarChart3}/><Card titulo="Avaliação" valor={retencao.feedback_average || '—'} detalhe={`${numero(retencao.feedback_total)} respostas`} icon={Eye}/></section>
