...
import { Home, BarChart2, Radio, Trophy, Crown, Star, ChevronRight, Menu, X, User, Zap, TrendingUp, Crosshair } from 'lucide-react';
...
const precisionData = [
    {dia:"Seg", valor:81}, {dia:"Ter", valor:83}, {dia:"Qua", valor:79},
    {dia:"Qui", valor:87}, {dia:"Sex", valor:85}, {dia:"Sab", valor:89}, {dia:"Dom", valor:87}
];
...
const mockJogoDetalhes = {
  ...
  tatH: "4-3-3", tatA: "4-2-3-1", coachH: "Tite", coachA: "Abel Ferreira",
  linH: [], linA: [], subsH: [], subsA: []
};

// 🔥 MOTOR MATEMÁTICO IA ORIGINAL
...
export default function App() {
  ...
  const carregarDadosEsporte = async (forcar = false) => {
    ...
    if (MODO_DEMONSTRACAO) {
        setTimeout(() => { aplicarFiltros(mockJogosData, ligaAtivaId); setLoading(false); }, 600); return;
    }
  ...
  if (showSplash) {
    return (
      <div className="flex flex-col justify-center items-center min-h-screen bg-[#050816] text-white">
         ...
         <p className="mt-4 text-slate-500 text-xs animate-pulse">Carregando jogos e IA...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#050816] text-white font-sans pb-28">
      ...
      <header className="flex items-center justify-between px-5 py-4">
        ...
        {/* 2. VIP ATIVO */}
        <button onClick={() => setMenuAtivo('assinar pro')} className="bg-yellow-500 hover:bg-yellow-400 text-black font-bold px-3 py-1.5 rounded-xl flex flex-col items-center shadow-lg shadow-yellow-500/20 transition-all">
          <div className="flex items-center gap-1 text-sm"><Crown className="w-4 h-4" /> {userData?.is_vip ? "VIP ATIVO" : "VIP"}</div>
          {userData?.is_vip && <span className="text-[9px] font-black opacity-80">Expira em 27 dias</span>}
        </button>
      </header>
          ...
          <div className="animate-fade-in">
              ...
              {viewMode === 'jogos' && (
                  <>
                    {/* 1. HERO PREMIUM */}
                    {userData?.is_vip && (
                        <div className="mx-4 mb-4 rounded-3xl p-5 bg-gradient-to-br from-blue-600 to-blue-900 shadow-[0_0_30px_rgba(13,110,253,0.3)] flex justify-between items-center relative overflow-hidden">
                        <div className="relative z-10">
                            <h2 className="text-lg font-black text-white flex items-center gap-2"><Crown className="w-5 h-5 text-yellow-400"/> IA Premium</h2>
                            <p className="text-blue-100 text-[11px] mt-2"><strong>87%</strong> de precisão hoje</p>
                            <p className="text-blue-100 text-[11px]"><strong>{jogos.length}</strong> jogos analisados</p>
                            <p className="text-yellow-300 text-[11px] font-bold"><strong>2</strong> oportunidades encontradas</p>
                        </div>
                        <button className="relative z-10 bg-white/20 hover:bg-white/30 backdrop-blur-md border border-white/30 text-white text-[10px] font-bold px-3 py-2 rounded-xl transition-all">
                            VER ANÁLISES
                        </button>
                        </div>
                    )}

                    {/* 3. OPORTUNIDADE DO DIA (Destaque) */}
                    <div className="mx-4 mb-6 rounded-3xl p-5 backdrop-blur-xl bg-slate-900/85 border border-white/10 shadow-lg relative overflow-hidden">
                        <div className="text-[9px] font-black text-red-500 bg-red-500/20 px-2 py-1 rounded-md inline-flex items-center gap-1 mb-3">🔥 OPORTUNIDADE DO DIA</div>
                        <h3 className="text-lg font-bold text-white mb-3">Flamengo x Palmeiras</h3>
                        <div className="flex justify-between items-end">
                            <div>
                                <div className="text-xs text-slate-400">Confiança IA: <strong className="text-green-400 text-sm">92%</strong></div>
                                <div className="text-[11px] text-slate-400 mt-1">Mercado: <strong className="text-white">Vitória Flamengo</strong></div>
                            </div>
                            <button onClick={() => abrirPainelDoJogo(mockJogosData[0])} className="bg-blue-600 hover:bg-blue-500 text-white text-[10px] font-bold px-4 py-2 rounded-xl shadow-lg shadow-blue-500/30">VER ANÁLISE</button>
                        </div>
                    </div>

                    {/* ESTATÍSTICAS RÁPIDAS */}
                    <div className="grid grid-cols-2 gap-3 px-4 mb-6">
                        <div className="backdrop-blur-xl bg-slate-900/85 border border-white/10 rounded-3xl p-4 flex flex-col items-center justify-center shadow-lg">
                            <div className="text-2xl font-black text-blue-500">{jogos.length}</div>
                            <div className="text-[10px] text-slate-400 font-bold uppercase mt-1">Jogos Hoje</div>
                        </div>
                        <div className="backdrop-blur-xl bg-slate-900/85 border border-white/10 rounded-3xl p-4 flex flex-col items-center justify-center shadow-lg">
                            <div className="text-2xl font-black text-red-500">{jogos.filter(j => j.status === 'Live').length}</div>
                            <div className="text-[10px] text-slate-400 font-bold uppercase mt-1">Ao Vivo</div>
                        </div>
                    </div>

                    {/* 4. GRÁFICO IA */}
                    <div className="mx-4 mb-6 rounded-3xl p-5 backdrop-blur-xl bg-slate-900/85 border border-white/10 shadow-lg">
                        <h3 className="font-bold text-sm text-white flex items-center gap-2 mb-4"><TrendingUp className="w-4 h-4 text-blue-500"/> Precisão IA (Últimos 7 dias)</h3>
                        <ResponsiveContainer width="100%" height={140}>
                            <LineChart data={precisionData}>
                            <XAxis dataKey="dia" stroke="#64748b" fontSize={10} tickLine={false} axisLine={false}/>
                            <YAxis domain={['dataMin - 5', 'dataMax + 5']} hide />
                            <Line type="monotone" dataKey="valor" stroke="#3b82f6" strokeWidth={3} dot={{r:4, fill:"#3b82f6", stroke:"#fff", strokeWidth:2}} />
                            </LineChart>
                        </ResponsiveContainer>
                    </div>
                    ...
                    {/* JOGOS LISTA (COM MINI STATS E CONFIANÇA) */}
                    <div className="px-4">
                        ...
                        Object.entries(jGrp).map(([leagueName, matches]) => (
                            <div key={leagueName} className="mb-6">
                                {matches.map(j => {
                                    ...
                                    // 6. CONFIANÇA IA (Fictícia para a lista)
                                    const randomConf = Math.floor(Math.random() * (95 - 75 + 1) + 75); 
                                    return (
                                        <div key={j.id} onClick={() => abrirPainelDoJogo(j)} className="backdrop-blur-xl bg-slate-900/85 border border-white/10 rounded-3xl p-4 shadow-xl mb-4 cursor-pointer relative overflow-hidden">
                                            ...
                                            {userData?.is_vip ? (
                                                <>
                                                    {/* 6. CONFIANÇA IA */}
                                                    <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-2 mb-3 flex items-center justify-between">
                                                        <span className="text-[10px] font-bold text-green-400">🟢 {randomConf}% Confiança</span>
                                                        <span className="text-[10px] text-slate-300">IA: <strong className="text-white">Vitória {j.home_team}</strong></span>
                                                    </div>

                                                    {/* 5. MINI STATS */}
                                                    {isLive && (
                                                        <div className="flex justify-between items-center text-[9px] text-slate-400 bg-slate-800/40 rounded-lg p-2 mb-3">
                                                            <div className="flex flex-col items-center"><span>Posse</span><strong className="text-white text-[10px]">62% - 38%</strong></div>
                                                            <div className="flex flex-col items-center"><span>Finalizações</span><strong className="text-white text-[10px]">12 - 7</strong></div>
                                                            <div className="flex flex-col items-center"><span>Escanteios</span><strong className="text-white text-[10px]">8 - 4</strong></div>
                                                        </div>
                                                    )}

                                                    <div className="grid grid-cols-3 gap-2">
                                                        {['1.82', '3.60', '4.75'].map((odd, idx) => (
                                                            <button key={idx} className="bg-slate-800/50 rounded-xl py-2 flex flex-col items-center justify-center border border-slate-700 hover:bg-blue-600 hover:border-blue-500 transition-colors">
                                                                <span className="text-[10px] text-slate-500 mb-0.5">{idx===0?'1':idx===1?'X':'2'}</span>
                                                                <span className="font-bold text-sm text-white">{odd}</span>
                                                            </button>
                                                        ))}
                                                    </div>
                                                </>
              ...
              {viewMode === 'ranking' && (
                  <div className="px-4">
                      ...
                      <div className="flex flex-col gap-3">
                          {[
                              {rank: 1, team: "Flamengo", conf: 92, match: "FLA x PAL"},
                              {rank: 2, team: "Real Madrid", conf: 89, match: "RMA x BAR"},
                              {rank: 3, team: "Liverpool", conf: 85, match: "LIV x MCI"},
                              {rank: 4, team: "Bayern Munich", conf: 81, match: "BAY x ARS"}
                          ].map(item => (
                              <div key={item.rank} className="backdrop-blur-xl bg-slate-900/85 border border-white/10 p-4 rounded-3xl flex items-center justify-between shadow-lg">
                                  <div className="flex items-center gap-4">
                                      <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black text-lg border ${item.rank === 1 ? 'bg-yellow-500/20 text-yellow-500 border-yellow-500/50' : item.rank === 2 ? 'bg-slate-400/20 text-slate-300 border-slate-400/50' : 'bg-orange-700/20 text-orange-400 border-orange-700/50'}`}>{item.rank}</div>
                                      <div>
                                          <div className="font-bold text-sm text-white">{item.team}</div>
                                          <div className="text-[10px] text-slate-400 mt-0.5">{item.match}</div>
                                      </div>
                                  </div>
                                  <div className="text-right">
                                      <div className="text-lg font-black text-green-400">{item.conf}%</div>
                                      <div className="text-[9px] text-slate-500 uppercase font-bold">Confiança</div>
                                  </div>
                              </div>
                          ))}
                      </div>
              ...
              {viewMode === 'perfil' && (
                  <div className="px-4">
                      ...
                      <div className="backdrop-blur-xl bg-slate-900/85 border border-white/10 p-6 rounded-3xl shadow-xl flex flex-col items-center text-center">
                          ...
                          {/* Nome personalizado com base no User Data */}
                          <h2 className="text-xl font-black text-white mb-1">👤 {form.nome || userData?.nome || "Lucas"}</h2>
                          
                          {userData?.is_vip ? (
                            <div className="text-[10px] font-black text-yellow-500 bg-yellow-500/10 px-4 py-1.5 rounded-full border border-yellow-500/20 mb-6">PLANO VIP PRO ATIVO</div>
                          ) : (
                            <div className="text-[10px] font-black text-slate-400 bg-slate-800 px-4 py-1.5 rounded-full border border-slate-700 mb-6">PLANO GRATUITO</div>
                          )}
                          ...
                          <div className="grid grid-cols-3 gap-3 w-full">
                              <div className="bg-[#050816] p-4 rounded-2xl border border-white/5">
                                  ...
                                  <div className="text-[9px] text-slate-500 font-bold uppercase mt-1">Análises</div>
                              </div>
                              <div className="bg-[#050816] p-4 rounded-2xl border border-white/5">
                                  <div className="text-xl font-black text-yellow-400">{favoritos.length}</div>
                                  <div className="text-[9px] text-slate-500 font-bold uppercase mt-1">Favoritos</div>
                              </div>
                              <div className="bg-[#050816] p-4 rounded-2xl border border-white/5">
                                  <div className="text-xl font-black text-green-400">84%</div>
                                  <div className="text-[9px] text-slate-500 font-bold uppercase mt-1">Precisão</div>
                              </div>
                          </div>
                      </div>

                      <div className="mt-6 flex flex-col gap-3">
                          <button className="backdrop-blur-xl bg-slate-900/85 border border-white/10 p-4 rounded-2xl text-sm font-bold text-left flex justify-between items-center">
                              Configurações da Conta <ChevronRight className="w-4 h-4 text-slate-500"/>
                          </button>
                          <button className="backdrop-blur-xl bg-slate-900/85 border border-white/10 p-4 rounded-2xl text-sm font-bold text-left flex justify-between items-center">
                              Suporte VIP <ChevronRight className="w-4 h-4 text-slate-500"/>
                          </button>
                      </div>
                  </div>
              )}
      ...
      {jogoSelecionado && !jogoSelecionado.is_loading && menuAtivo !== 'assinar pro' && (
        <motion.div initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} className="px-4">
            <button onClick={() => setJogoSelecionado(null)} className="text-slate-400 text-xs font-bold flex items-center gap-1 mb-4 backdrop-blur-xl bg-slate-900/85 border border-white/10 px-4 py-2 rounded-xl"><X className="w-4 h-4"/> Fechar Análise</button>
            
            <div className="backdrop-blur-xl bg-slate-900/85 rounded-3xl p-6 border border-blue-500/30 shadow-2xl shadow-blue-500/10 mb-6">
                <div className="flex justify-between items-center mb-6">
                    <div className="flex flex-col items-center w-1/3">
                        <img src={jogoSelecionado.home_image} className="w-14 h-14 mb-2 drop-shadow-lg" alt=""/>
                    ...
                    <div className="w-1/3 text-center">
                        ...
                        <span className="text-[10px] text-red-500 font-bold bg-red-500/10 px-2 py-1 rounded-md">{jogoSelecionado.time_elapsed}' MIN</span>
                    </div>
                    <div className="flex flex-col items-center w-1/3">
                        <img src={jogoSelecionado.away_image} className="w-14 h-14 mb-2 drop-shadow-lg" alt=""/>
                ...
                <div className="bg-[#050816] rounded-2xl p-5 border border-slate-800/80 relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-20 h-20 bg-blue-600/10 blur-xl rounded-full"></div>
                    <div className="flex items-center gap-2 mb-3 relative z-10">
                        <Crosshair className="w-5 h-5 text-blue-500" />
                        <h4 className="font-black text-xs text-blue-400">ANÁLISE ESTATÍSTICA IA</h4>
                    </div>
            ...
            <div className="backdrop-blur-xl bg-slate-900/85 rounded-3xl p-6 border border-white/10 shadow-xl">
                <h4 className="font-black text-sm mb-4 text-white">ESTATÍSTICAS DO CONFRONTO</h4>
                    ...
                    return (
                        <div key={i} className="mb-4">
                            <div className="flex justify-between text-xs font-bold mb-1">
                                ...
                                <span className="text-slate-500 text-[10px] uppercase">{s.type}</span>
                                <span className={pA>=pH ? 'text-yellow-400' : 'text-slate-500'}>{s.a}</span>
                            </div>
                            <div className="flex gap-1 h-1.5 rounded-full overflow-hidden bg-[#050816]">
      ...
      {jogoSelecionado?.is_loading && (
          <div className="px-4 py-10 flex flex-col items-center justify-center h-64">
              ...
              <p className="text-blue-500 font-bold text-xs animate-pulse">A extrair dados do algoritmo...</p>
          </div>
      )}
      ...
      {menuAtivo === "assinar pro" && (
         <motion.div initial={{opacity:0}} animate={{opacity:1}} className="px-4 pt-4">
             ...
             <div className="backdrop-blur-xl bg-slate-900/85 rounded-3xl p-6 border border-white/10 shadow-2xl">
                ...
                <div className="flex flex-col gap-3">
                    <input placeholder="Nome" value={form.nome} onChange={e=>setForm({...form,nome:e.target.value})} className="bg-[#050816] border border-slate-800 p-4 rounded-2xl text-sm outline-none focus:border-blue-500 transition-colors text-white" />
                    <input placeholder="E-mail" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} className="bg-[#050816] border border-slate-800 p-4 rounded-2xl text-sm outline-none focus:border-blue-500 transition-colors text-white" />
                    <input placeholder="CPF" maxLength={11} value={form.cpf} onChange={e=>setForm({...form,cpf:e.target.value.replace(/\D/g, '')})} className="bg-[#050816] border border-slate-800 p-4 rounded-2xl text-sm outline-none focus:border-blue-500 transition-colors text-white" />
                    
                    <button className="mt-4 bg-green-500 hover:bg-green-600 text-white font-black py-4 rounded-2xl shadow-lg shadow-green-500/20 transition-all text-sm">
                        ⚡ PAGAR 29,90€ COM PIX
                    </button>
                    <button className="bg-[#050816] border border-slate-800 hover:bg-slate-800 text-white font-bold py-4 rounded-2xl transition-all text-sm">
                        💳 Cartão de Crédito
                    </button>
                </div>
             </div>
      ...
      <AnimatePresence>
          {aiOpen && (
              <motion.div initial={{opacity:0, y:20, scale:0.9}} animate={{opacity:1, y:0, scale:1}} exit={{opacity:0, scale:0.9, y:20}} className="fixed right-4 left-4 bottom-24 backdrop-blur-2xl bg-slate-900/95 border border-slate-700 p-5 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.8)] z-50">
                  <div className="flex justify-between items-center mb-4">
                      <h3 className="font-black flex items-center gap-2 text-white"><Zap className="w-4 h-4 text-yellow-400"/> Assistente IA</h3>
                      <button onClick={() => setAiOpen(false)} className="text-slate-400 bg-slate-800 rounded-full p-1"><X className="w-4 h-4"/></button>
                  </div>
                  <div className="bg-[#050816] border border-slate-800 rounded-2xl p-4 mb-4 text-xs text-slate-300 leading-relaxed relative">
                      <div className="absolute top-4 left-0 w-1 h-full bg-blue-500 rounded-r-md"></div>
                      Olá, {form.nome || userData?.nome || "Apostador"}! Sou o motor BetAnalytics. Qual é a sua dúvida sobre a rodada de hoje?
                  </div>
                  <input type="text" placeholder="Ex: Qual a melhor aposta para o Flamengo?" value={aiQuery} onChange={(e)=>setAiQuery(e.target.value)} className="w-full bg-[#050816] border border-slate-700 rounded-xl p-3 text-xs text-white mb-3 outline-none focus:border-blue-500"/>
                  <button className="w-full bg-blue-600 text-white font-bold text-xs py-3 rounded-xl hover:bg-blue-500 transition-colors shadow-lg shadow-blue-500/20">Perguntar à IA</button>
              </motion.div>
          )}
      </AnimatePresence>

      {/* 11. NOVA BOTTOM NAVIGATION */}
      <nav className="fixed bottom-0 left-0 right-0 h-20 bg-slate-900/90 border-t border-white/5 flex justify-around items-center backdrop-blur-xl z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
        ...
        <button onClick={() => {setViewMode('blog'); setJogoSelecionado(null);}} 
          ...
          <span className="text-[9px] font-black uppercase">Análises</span>
        </button>
        ...
        <button onClick={() => {setViewMode('jogos'); setFilterCentro('Ao Vivo'); setMenuAtivo('Todos os Jogos'); setJogoSelecionado(null);}} 
          ...
          <span className="text-[9px] font-black uppercase">Ao Vivo</span>
        </button>

        {/* NOVA ABA: RANKING */}
        <button onClick={() => {setViewMode('ranking'); setJogoSelecionado(null);}} 
                className={`flex flex-col items-center gap-1.5 transition-colors ${viewMode === 'ranking' ? 'text-yellow-500' : 'text-slate-500 hover:text-slate-300'}`}>
          <Trophy className="w-6 h-6" />
          <span className="text-[9px] font-black uppercase">Ranking</span>
        </button>
