...
const T = {
  ...
  sao: { id: 1004, name: 'São Paulo', logo: logoSvg('São Paulo', '#dc2626', '#111827') },
};
...
function events(home, away, p = 1) {
  return p === 1 ? [
    ...
    { time:{elapsed:58}, team:home, player:{id:4,name:'Bruno Henrique'}, assist:{name:'Luiz Araújo'}, type:'Goal', detail:'Normal Goal' },
  ] : [
...
function lineup(team, side) {
  const names = side === 'home'
    ? ['Rossi','Varela','Léo Ortiz','Léo Pereira','Ayrton Lucas','Pulgar','Gerson','Arrascaeta','Luiz Araújo','Bruno Henrique','Pedro']
    : ['Weverton','Mayke','Gómez','Murilo','Piquerez','Aníbal Moreno','Zé Rafael','Raphael Veiga','Estêvão','Rony','Flaco López'];
  const grids = ['1:1','2:1','2:2','2:4','2:5','3:2','3:4','4:3','4:1','4:5','5:3'];
  return { team, formation:'4-2-3-1', coach:{ name: side === 'home' ? 'Técnico Mandante' : 'Técnico Visitante' }, startXI:names.map((name,i)=>({ player:{ id:team.id*10+i, name, number:i+1, pos:i===0?'G':i<5?'D':i<8?'M':'F', grid:grids[i] }})), substitutes:[] };
}

function players(team, side) {
  const names = side === 'home' ? ['Pedro','Arrascaeta','Gerson','Bruno Henrique','Luiz Araújo'] : ['Raphael Veiga','Rony','Gómez','Estêvão','Piquerez'];
...
function oddsLive(h,a) { return [{ bookmaker:'Live Demo', odds:[{ name:'Resultado ao vivo', values:[{value:h,odd:'1.62'},{value:'Empate',odd:'3.90'},{value:a,odd:'5.40'}] },{ name:'Próximo gol', values:[{value:h,odd:'1.95'},{value:'Sem gol',odd:'4.60'},{value:a,odd:'2.75'}] }] }]; }
function h2h(home, away) { return [
  { fixture:{id:8001,status:{short:'FT',long:'Match Finished'}}, league:{name:'Brasileirão Série A',season:2025}, teams:{home,away}, goals:{home:2,away:1} },
